...
public class PlayerJoinListener implements Listener {
    ...
    public void onPlayerJoin(PlayerJoinEvent event) {
        ...
        // Anti-VPN check
        if (ip != null && plugin.getVPNManager() != null && plugin.getVPNManager().isEnabled()) {
            if (!player.hasPermission("litebansreborn.bypass.vpn")) {
    ...
    /**
     * Check for VPN/Proxy and take action
     */
    private void checkVPN(Player player, UUID uuid, String name, String ip) {
        plugin.getVPNManager().checkIP(ip).thenAccept(result -> {
            if (result == null || !result.isDangerous()) {
                // Not a VPN, track clean IP
                plugin.getVPNManager().getDatabase().trackIP(uuid, name, ip, false);
                return;
            }
            
            // VPN detected!
            com.nuvik.litebansreborn.antivpn.VPNManager.VPNAction action = plugin.getVPNManager().getAction();
            ...
            if (plugin.getConfigManager().getBoolean("anti-vpn.notifications.log-to-console", true)) {
                plugin.log(java.util.logging.Level.WARNING, 
                    "§c[AntiVPN] §e" + name + " §7(" + ip + ") - VPN detected: " + 
            ...
            Bukkit.getScheduler().runTask(plugin, () -> {
                ...
                switch (action) {
                    ...
                    case WARN -> {
                        // Notify staff only
                        notifyStaffVPN(player, result);
                    }
                    case ALLOW -> {
                        // Allow but notify staff
                        notifyStaffVPN(player, result);
                    }
                    case NONE -> {
                        // Do nothing
    ...
    /**
     * Notify staff about VPN detection
     */
    private void notifyStaffVPN(Player vpnPlayer, com.nuvik.litebansreborn.antivpn.VPNResult result) {
